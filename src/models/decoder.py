...
class TransformerDecoder(keras.Model):
    ...
    def call(self, captions, encoder_output):
        """
        captions: [batch, seq_len] token ids for decoder input
        encoder_output: output of encoder (e.g. image features) to attend to
        """

        x = self.embedding(captions)
        x = self.pos_encoding(x)  
        out = self.transformer_block(x, encoder_output)s
